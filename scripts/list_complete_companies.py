import os
import glob
import json
import csv
from tqdm import tqdm
from concurrent.futures import ThreadPoolExecutor, as_completed

TARGET_DIR = r"c:\dev\edinet2dataset\edinet_corpus\annual"
OUTPUT_FILE = r"c:\dev\edinet2dataset\complete_companies.csv"
THRESHOLD = 10  # 10年分以上を抽出

def process_company(company_path):
    try:
        edinet_code = os.path.basename(company_path)
        tsv_files = glob.glob(os.path.join(company_path, "*.tsv"))
        count = len(tsv_files)
        
        # 10年分未満はスキップ
        if count < THRESHOLD:
            return None

        # 会社名を取得
        company_name = "Unknown"
        json_files = glob.glob(os.path.join(company_path, "*.json"))
        
        if json_files:
            try:
                # 複数ある場合は新しいものを優先的にトライしたいが、
                # ここではシンプルに最初のファイルを読む。
                # ただし、読み込み失敗してもUnknownとして返すようにする（Noneを返さない）。
                with open(json_files[0], 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    company_name = data.get('filerName', 'Unknown')
            except Exception as e:
                # JSON読み込みエラーでもスキップせず、Unknownとして登録
                # print(f"Error reading JSON for {edinet_code}: {e}") 
                pass
        
        return {
            "edinet_code": edinet_code,
            "company_name": company_name,
            "file_count": count
        }
    except Exception as e:
        print(f"Critical error processing {company_path}: {e}")
        return None

def main():
    if not os.path.exists(TARGET_DIR):
        print(f"Directory not found: {TARGET_DIR}")
        return

    company_dirs = [os.path.join(TARGET_DIR, d) for d in os.listdir(TARGET_DIR) if os.path.isdir(os.path.join(TARGET_DIR, d))]
    print(f"Scanning {len(company_dirs)} companies for those with {THRESHOLD} or more years of data...")

    complete_companies = []
    
    with ThreadPoolExecutor(max_workers=16) as executor:
        futures = [executor.submit(process_company, p) for p in company_dirs]
        
        for future in tqdm(as_completed(futures), total=len(futures)):
            result = future.result()
            if result:
                complete_companies.append(result)

    with open(OUTPUT_FILE, 'w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=["edinet_code", "company_name", "file_count"])
        writer.writeheader()
        complete_companies.sort(key=lambda x: x['edinet_code'])
        writer.writerows(complete_companies)

    print(f"Found {len(complete_companies)} companies with {THRESHOLD} or more files.")
    print(f"Saved list to {OUTPUT_FILE}")

if __name__ == "__main__":
    main()
